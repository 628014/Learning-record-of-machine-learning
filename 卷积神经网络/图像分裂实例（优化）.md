...
另一种减少过度拟合的技术是将 Dropout 引入网络，这是一种正则化形式。将 Dropout 应用于一个层时，它会在训练过程中从该层中随机删除（通过将激活设置为零）许多输出单元。 Dropout 将一个小数作为其输入值，形式为 0.1、0.2、0.4 等。这意味着从应用层中随机丢弃 10%、20% 或 40% 的输出单元。使layers.Dropout 创建一个新的神经网络，然后使用增强图像训练它。
